...
def robust_normalize_tex(tex: str) -> str:
	...
	for _ in range(5):
		...
		tex = _unwrap_delimiter_wrapped_group(tex, "[", "]")
		tex = _unwrap_delimiter_wrapped_group(tex, "|", "|")
		tex = _unwrap_delimiter_wrapped_group(tex, "\\langle", "\\rangle")
		tex = tex.replace("\\{{", "\\{").replace("}\\}", "\\}")
		tex = tex.replace("{}", "") # 删除用于结构占位的空括号(改变二元/一元性质)
		tex = tex.replace("_{}", "")
		tex = tex.replace("^{}", "")
		tex = _unwrap_double_braces(tex)
	
	tex = _unwrap_outer_brace(tex)
...
def canonicalize_svg(svg_text: str) -> Optional[str]:
	...
	def _serialize_elem(elem: ET.Element) -> str:
		...
		if tag == "g" and not attrs and not (elem.text and elem.text.strip()):
			pieces = []
			for child in list(elem):
...
def collate_eval_batch(batch, pad_token_id: int) -> Dict[str, object]:
